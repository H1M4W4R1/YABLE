...
def render_characteristic_card(app: tk.Tk, parent: ttk.Frame, characteristic: CharacteristicModel) -> None:
    ...
    if "read" in characteristic.properties:
        ttk.Button(right, text="Read", command=lambda char=characteristic: app.bridge.read_characteristic(char.characteristic), style="Ghost.TButton").pack(side="left", padx=3)
    if "write" in characteristic.properties or "write-without-response" in characteristic.properties:
        ttk.Button(right, text="Write", command=lambda char=characteristic: app._open_write_dialog(char), style="Accent.TButton").pack(side="left", padx=3)
